...
import { TrendChart, ProductBars, Sparkline } from "@/components/dashboard-charts";
import {
  summarize,
  dailyTotals,
  topProducts,
...
async function loadSales(shopId: string, since: string): Promise<DashboardSale[]> {
  ...
  return (rows ?? []).map((r) => ({
    ...
    seller_name: (r.profiles as unknown as { full_name: string } | null)?.full_name ?? "Unknown",
...
export default async function AdminDashboardPage({
  ...
  const rangeDays: RangeDays = RANGE_OPTIONS.includes(requested as RangeDays) ? (requested as RangeDays) : 30;
  ...
  const prevAvgTicket = prevTotals.receipts ? prevTotals.gross / prevTotals.receipts : 0;
    ...
    <div className="px-6 py-6 md:px-10">
      <div className="mb-6 flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-[13px] text-ink-soft">{session.shop.business_name}</p>
        </div>
            ...
            {RANGE_OPTIONS.map((opt) => (
              <Link
                ...
                className={`rounded-md px-3 py-1.5 text-xs font-bold ${
                  opt === rangeDays ? "bg-surface-2 text-foreground" : "text-ink-soft hover:text-foreground"
                }`}
              >
                {opt}d
              </Link>
      ...
      <div className="mb-3 grid grid-cols-2 gap-3 lg:grid-cols-4">
        {kpis.map((k) => {
          const up = k.delta >= 0;
          return (
            <div key={k.label} className="rounded-xl border border-line bg-surface p-4 shadow-sm">
              <div className="text-[11px] font-bold uppercase tracking-wide text-ink-soft">{k.label}</div>
              <div className="num mt-1.5 text-xl font-semibold">{k.value}</div>
              <div className={`mt-1 flex items-center gap-1 text-xs font-bold ${up ? "text-good" : "text-critical"}`}>
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={3} className="h-2.5 w-2.5 flex-none">
                  {up ? <path d="M4 16l7-8 5 5 4-6M15 7h5v5" /> : <path d="M4 8l7 8 5-5 4 6M15 17h5v-5" />}
                </svg>
                <span className="truncate">{Math.abs(k.delta).toFixed(1)}% vs prior {rangeDays}d</span>
              </div>
      ...
      <div className="mb-3 grid gap-3 lg:grid-cols-[1.6fr_1fr]">
        <section className="rounded-xl border border-line bg-surface p-4 shadow-sm">
          <div className="mb-3 flex flex-wrap items-start justify-between gap-2">
            <div>
              ...
              <p className="text-[11.5px] text-ink-soft">Net value after VAT vs. output VAT, by sale date</p>
            </div>
            <div className="flex gap-3 text-[11.5px] font-semibold text-ink-soft">
              <span className="flex items-center gap-1.5">
                <span className="h-2 w-2 rounded-sm" style={{ background: "var(--color-s1)" }} />
                Net sales
              </span>
              <span className="flex items-center gap-1.5">
                <span className="h-2 w-2 rounded-sm" style={{ background: "var(--color-s2)" }} />
                VAT collected
              </span>
            </div>
          </div>
          {daily.some((d) => d.net > 0) ? (
            ...
            <p className="py-10 text-center text-sm text-ink-soft">
              No sales recorded in this range yet — receipts synced from VatFlow Mobile will appear here.
            </p>
          )}
        </section>
        ...
        <section className="rounded-xl border border-line bg-surface p-4 shadow-sm">
          ...
          <div className="flex h-3.5 overflow-hidden rounded-full border border-line">
            <span style={{ width: `${split.total ? (split.g / split.total) * 100 : 0}%`, background: "var(--color-s1)" }} />
            <span style={{ width: `${split.total ? (split.s / split.total) * 100 : 0}%`, background: "var(--color-s3)" }} />
          </div>
          <div className="mt-2.5 space-y-1.5">
            <SplitRow color="var(--color-s1)" label="G — VAT applies" count={split.g} total={split.total} />
            <SplitRow color="var(--color-s3)" label="S — VAT exempt/special" count={split.s} total={split.total} />
          </div>
        </section>
      </div>
      ...
      <section className="mb-3 rounded-xl border border-line bg-surface shadow-sm">
        <div className="p-4 pb-0">
          ...
          <p className="mb-3 text-[11.5px] text-ink-soft">Receipts recorded in the selected range, newest first</p>
        </div>
              ...
              {recentSales.map((sale) => (
                <tr key={sale.id} className="border-b border-line last:border-0 hover:bg-surface-2">
                  ...
                  <td className="px-4 py-2.5">
                    <div className="font-semibold">{sale.buyer_name ?? "Walk-in customer"}</div>
                    {sale.buyer_tin && <div className="text-[11px] text-ink-faint">TIN {sale.buyer_tin}</div>}
                  </td>
                  <td className="px-4 py-2.5">
                    <span
                      className="inline-flex h-5 w-5 items-center justify-center rounded font-data text-[10.5px] font-bold"
                      style={{
                        background: sale.vat_category === "G" ? "color-mix(in srgb, var(--color-s1) 20%, transparent)" : "color-mix(in srgb, var(--color-s3) 20%, transparent)",
                        color: sale.vat_category === "G" ? "var(--color-s1)" : "var(--color-s3)",
                      }}
                    >
                      {sale.vat_category}
                    </span>
                  ...
                  <td className="num px-4 py-2.5 text-right">{saleGross(sale).toFixed(2)} ETB</td>
                  <td className="px-4 py-2.5">
                    <span
                      className={`rounded-full px-2 py-0.5 text-[11px] font-bold ${
                        sale.client_id ? "bg-good-bg text-good" : "bg-surface-2 text-ink-soft"
                      }`}
                    >
          ...
          {sellers.map((s) => (
            <div key={s.name} className="flex items-center gap-3 py-2.5">
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="text-[11.5px] text-ink-soft">{s.receipts} receipts</div>
                <div className="mt-1 h-1 overflow-hidden rounded-full bg-surface-2">
                  <div className="h-full rounded-full bg-brand" style={{ width: `${(s.value / maxSellerValue) * 100}%` }} />
                </div>
              </div>
              <div className="num flex-none text-[13px] font-semibold">{s.value.toFixed(2)} ETB</div>
            </div>
          ))}
          {sellers.length === 0 && <p className="py-4 text-sm text-ink-soft">No sales recorded yet.</p>}
        </div>
      </section>

      <p className="mt-6 text-center text-xs text-ink-faint">
        {formatEthiopianDate(today)} E.C. · Sales are recorded on VatFlow Mobile and synced here automatically.
      </p>
    </div>
  );
}

function SplitRow({ color, label, count, total }: { color: string; label: string; count: number; total: number }) {
